...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f System::TrackRGBDByYOLO(const cv::Mat &im, const cv::Mat &depthmap,const cv::Mat &mask, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
    {
        if(mSensor!=RGBD  && mSensor!=IMU_RGBD)
        {
            cerr << "ERROR: you called TrackRGBD but input sensor was not set to RGBD." << endl;
        ...
        //Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(imToFeed,imDepthToFeed,timestamp,filename);
        Sophus::SE3f Tcw = mpTracker->GrabImageRGBDYOLO(imToFeed,imDepthToFeed,mask,timestamp,filename);
        unique_lock<mutex> lock2(mMutexState);
        mTrackingState = mpTracker->mState;
        mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints;
        mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn;
