...
void PointCloudMapping::generatePointCloud(KeyFrame *kf) //,Eigen::Isometry3d T
{
    ...
    uchar pixel_value ;
    //// Debug 语义稠密建图 输入octmap生成栅格 用作A*导航
    for (int m = 0; m < kf->imDepth.rows; m += 4)
    {
        for (int n = 0; n < kf->imDepth.cols; n += 4)
        {
            float d = kf->imDepth.ptr<float>(m)[n];
            for(int m_ = -20 ; m_ < 20 ; m_++)
            {
                for(int n_ = -20 ; n_ < 20 ; n_++)
                {
                    if(m+m_ < kf->imDepth.rows && n+n_<kf->imDepth.cols)
                    {
                        pixel_value = kf->imMaskImage.ptr<uchar>(m+m_)[n+n_];
                        if(pixel_value != 0)
                        {
                            d = 0;
                        }
...
void PointCloudMapping::viewer()
{
    ...
    while (1)
    {
        ...
        for (auto pKF : lNewKeyFrames)
        {
            ...
            // gettimeofday(&finish,NULL);//初始化结束时间
            // transformPointCloudTime += finiORB3Yolo8sh.tv_sec - start.tv_sec + (finish.tv_usec - start.tv_usec)/1000000.0;
        }
        // gettimeofday(&start,NULL);
        ...
        //pcl::toROSMsg(*globalMap, pcl_point);
        ...
        pcl::io::savePCDFileBinary(save_path, *globalMap);
