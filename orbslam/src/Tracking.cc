...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    Sophus::SE3f Tracking::GrabImageRGBDYOLO(const cv::Mat &imRGB,const cv::Mat &imD,const cv::Mat &imMask, const double &timestamp, string filename)
    {
        mimLeft = imRGB.clone();
        mImGray = imRGB;
        mImDepth = imD.clone();
        ...
        if((fabs(mDepthMapFactor-1.0f)>1e-5) && mImDepth.type()!=CV_32F)
            mImDepth.convertTo(mImDepth,CV_32F,mDepthMapFactor);
        mImMaskTracking = imMask.clone();
        // Step 3：构造Frame
        if (mSensor == System::RGBD)
            mCurrentFrame = Frame(mImGray,mImDepth,imMask,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
        else if(mSensor == System::IMU_RGBD)
            mCurrentFrame = Frame(mImGray,mImDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO || mSensor==System::RGBD)
    {
        ...
        pKF->imMaskImage = mImMaskTracking.clone();
